        ...
        while (Time.time < endTime)
        {
            ...
            foreach (var collider in colliders)
            {
                if (collider.gameObject.TryGetComponent<EnemyHurt>(out EnemyHurt enemy))
                {
                    if (enemy != null)
                    {
                        GameObject cloneLighting = Instantiate(lightprefab, enemy.transform.position + spellOffSet, Quaternion.Euler(0, 0, 40f));
                        cloneLighting.transform.parent = enemy.transform;
